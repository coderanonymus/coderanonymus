...
build/hvcore.o: kernel/hypervisor/core.c | build
	$(CC) $(CFLAGS) -c $< -o $@

build/hvvmx.o: kernel/hypervisor/vmx.c | build
	$(CC) $(CFLAGS) -c $< -o $@

build/hvsvm.o: kernel/hypervisor/svm.c | build
	$(CC) $(CFLAGS) -c $< -o $@

build/sched2.o: kernel/sched2/cfs.c | build
	$(CC) $(CFLAGS) -c $< -o $@

build/mm2.o: kernel/mm/mm2.c | build
	$(CC) $(CFLAGS) -c $< -o $@

build/security.o: kernel/security/capability.c | build
	$(CC) $(CFLAGS) -c $< -o $@

build/container.o: kernel/container/container.c | build
	$(CC) $(CFLAGS) -c $< -o $@

build/net2.o: kernel/net2/stack.c | build
	$(CC) $(CFLAGS) -c $< -o $@

build/gfx2.o: kernel/gfx2/compositor.c | build
	$(CC) $(CFLAGS) -c $< -o $@

build/storage.o: kernel/storage/block.c | build
	$(CC) $(CFLAGS) -c $< -o $@

build/sdk.o: kernel/sdk/sdk.c | build
	$(CC) $(CFLAGS) -c $< -o $@

build/services_restart.o: kernel/services/restart.c | build
	$(CC) $(CFLAGS) -c $< -o $@
...
check-host:
	gcc -std=gnu11 -Wall -Wextra -Iinclude -fsyntax-only \
		...
		kernel/hypervisor/core.c kernel/hypervisor/vmx.c kernel/hypervisor/svm.c \
		kernel/sched2/cfs.c kernel/mm/mm2.c kernel/security/capability.c \
		kernel/container/container.c kernel/net2/stack.c kernel/gfx2/compositor.c \
		kernel/storage/block.c kernel/sdk/sdk.c kernel/services/restart.c \
